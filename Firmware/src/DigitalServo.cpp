...
uint8_t DigitalServo::attachedA = 0;
uint8_t DigitalServo::attachedB = 0;
#ifdef SERVO_PIN_C
uint8_t DigitalServo::attachedC = 0;
#endif

void DigitalServo::seizeTimer1()
{
...
void DigitalServo::releaseTimer1() {}

DigitalServo::DigitalServo() : pin(0), angle(NO_ANGLE) {}

uint8_t DigitalServo::attach(int pinArg, int min, int max)
{
  #ifdef SERVO_PIN_C
...
void DigitalServo::write(int angleArg)
{
  uint16_t p;
...
uint32_t DigitalServo::attachedpins[(NUM_DIGITAL_PINS+31)/32]; // 1 bit per digital pin

DigitalServo::DigitalServo() : pin(255), angle(NO_ANGLE) {}
